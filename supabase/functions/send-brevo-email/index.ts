...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { order_id, user_id, pickup_code, canteen_id, canteen_name } = payload

    console.log(`Processing email notification for order: ${order_id}`)
    ...
    if (existingEmail) {
      console.log(`Email already sent for order ${order_id}, skipping`)
      return new Response(
            ...
            <body style="font-family: Arial, sans-serif; line-height: 1.8; color: #333; background-color: #f9fafb;">
              <div style="max-width: 600px; margin: 0 auto; padding: 30px; background-color: #ffffff; border-radius: 12px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
                <div style="text-align: center; margin-bottom: 30px;">
                  <h1 style="color: #2563eb; margin: 0; font-size: 28px;">🎉 Your Order is Ready!</h1>
                </div>
                ...
                <p style="font-size: 16px; margin-bottom: 20px;">Your order with ID <strong style="color: #2563eb;">#${order_id.slice(0, 8).toUpperCase()}</strong> from <strong>${finalCanteenName}</strong> is now <strong style="color: #16a34a;">READY</strong> for pickup.</p>
                
                <div style="background: linear-gradient(135deg, #2563eb 0%, #1d4ed8 100%); padding: 25px; border-radius: 12px; margin: 25px 0; text-align: center;">
                ...
                <p style="font-size: 16px; margin-bottom: 30px;">
                  Regards,<br>
                  <strong>${finalCanteenName}</strong>
                </p>
...
Your order with ID #${order_id.slice(0, 8).toUpperCase()} from ${finalCanteenName} is now READY for pickup.
...
This is an automated message from PreOrder.`,
    ...
    console.log(`Email sent successfully for order ${order_id}:`, brevoResult)
