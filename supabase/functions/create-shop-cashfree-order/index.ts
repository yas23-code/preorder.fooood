...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('Creating Cashfree shop order:', { orderId, amount, customerName, customerEmail });
    ...
    const orderPayload = {
      order_id: `shop_${orderId}`,
      order_amount: amount,
      order_currency: 'INR',
      customer_details: {
        customer_id: orderId.split('-')[0],
    ...
    // Update shop order with payment_id in database
    ...
    if (updateError) {
      console.error('Error updating shop order with payment info:', updateError);
    }
    ...
    console.error('Error creating Cashfree shop order:', error);
    return new Response(
