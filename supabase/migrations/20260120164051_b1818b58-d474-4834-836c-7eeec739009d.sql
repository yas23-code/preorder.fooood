-- Add prep_time to menu_items table (in minutes)
ALTER TABLE public.menu_items 
ADD COLUMN prep_time integer DEFAULT NULL;

-- Add estimated_ready_time to orders table
ALTER TABLE public.orders 
ADD COLUMN estimated_ready_time timestamp with time zone DEFAULT NULL;

-- Create function to calculate ETA for an order
CREATE OR REPLACE FUNCTION public.calculate_order_eta(
  p_canteen_id uuid,
  p_item_ids uuid[]
)
...
BEGIN
  ...
  -- Count pending orders for this canteen
