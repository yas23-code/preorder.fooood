...
import { ClipboardList, Clock, CheckCircle, Package, Plus, LogOut, Tag, Search, X, IndianRupee, ShoppingBag, AlertCircle, XCircle, QrCode, Crown } from 'lucide-react';
...
export default function VendorDashboard() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const { data: canteenData } = await supabase
        .from('canteens')
        .select('id, name, location, is_open, order_limit, is_accepting_orders, approval_status, stock_mode, member_discount_value')
      ...
      setMemberDiscountInput(canteenData.member_discount_value?.toString() || '0');
  ...
  const handleUpdateMemberDiscount = async () => {
    if (!canteen) return;

    const discountValue = Number(memberDiscountInput);
    ...
    const { error } = await supabase
      .from('canteens')
      .update({ member_discount_value: discountValue })
      .eq('id', canteen.id);
    ...
    if (error) {
      ...
      setCanteen({ ...canteen, member_discount_value: discountValue });
      toast.success('Member discount updated!');
    }
    ...
    <div className="min-h-screen bg-mcd-cream">
      ...
      <main className="page-container">
        ...
        <div className="mb-6 md:mb-8 grid gap-4 md:grid-cols-2">
          ...
          <div className="bg-white rounded-xl border border-mcd-border p-4 shadow-card">
            <div className="flex items-center gap-2 mb-4">
              <div className="p-2 bg-mcd-red/10 rounded-lg">
                <Crown className="h-5 w-5 text-mcd-red" />
              </div>
              <div>
                <h3 className="font-bold text-mcd-text">Campus Club Discount</h3>
                <p className="text-[10px] text-mcd-text/60 italic">Applies only to eligible active members on orders ≥ ₹70</p>
              </div>
            </div>
            <div className="flex gap-2">
              <div className="relative flex-1">
                <IndianRupee className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-mcd-text/40" />
                <Input
                  type="number"
                  value={memberDiscountInput}
                  onChange={(e) => setMemberDiscountInput(e.target.value)}
                  className="pl-9 h-11 bg-mcd-selected border-mcd-border focus:border-mcd-red"
                  placeholder="Set base discount (₹)"
                />
              </div>
              <Button
                onClick={handleUpdateMemberDiscount}
                disabled={isUpdatingDiscount}
                className="h-11 bg-mcd-red hover:bg-red-700 text-white font-bold px-6"
              >
                Update
              </Button>
            </div>
